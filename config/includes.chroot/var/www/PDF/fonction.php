...
require('/var/www/PDF/fpdf.php');
include('/var/www/qos.inc.php');
...
function Header()
{
    ...
    $this->Image('/var/www/PDF/logo.png',10,6,30);
    // Arial gras 15
...
// X,
    $this->Image('/var/www/PDF/logo_partenaire.png',170,6,30);
}
...
function Footer()
{
    ...
    $this->Cell(10,6,"Document Confidentiel - Usage interne uniquement  ",0,0,'L');
...
function Recap($fichier,$NomClient,$Lieu,$NomTest,$NomTech,$Consigne, $paquet, $tpc, $tos, $ip_src, $ip_dst, $nego)
{
...
$NbLigne = count($file);
    ...
    $this->Cell(10,6,"Consigne : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$Consigne . " kbits",0,1,'L');

    $this->Cell(10,6,"IP_SRC : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$ip_src,0,1,'L');

    $this->Cell(10,6,"IP_DST : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$ip_dst,0,1,'L');

    $this->Cell(10,6,"ETH0 : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$nego,0,1,'L');

    $this->Cell(10,6,"Tailles paquets : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$paquet,0,1,'L');

    $this->Cell(10,6,"Tps par cycle : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$tpc . " secondes",0,1,'L');


    $str_tos = tos_to_dscp($tos);
    $this->Cell(10,6,"Type Of Service : ",0,0,'L');
    $this->Cell($decal);
    $this->Cell(10,6,$str_tos,0,1,'L');
...
function RecapQos($fichier,$NomClient,$Lieu,$NomTest,$NomTech,$tpc)
{
    // Lecture du fichier texte
$txt = file_get_contents($fichier);
$file = file($fichier);
...
$decal = "70";
    ...
    $this->Cell($decal);
    $this->Cell(10,6,"Datarate en RX  (bits/s)         (kbits/s)         (Mbits/s)",0,1,'L');
...
function get_tos_string($value)
{
	switch($value) {
		...
		case '80': return "AF22";
		case '96': return "CS3";
		case '104': return "AF31";
		case '112': return "AF32";
		case '128': return "CS4";
		case '136': return "AF41";
		case '144': return "AF42";
		case '184': return "EF";
		default: return "Valeur inconnues ('" . $value . "')";
	}
}
...
function FancyTable($header, $data)
{
    ...
    $this->SetFillColor(0,0,0); // rouge 255.0.0
...
// Tableau coloré
function FancyTable_debit($header, $data)
{
    // Couleurs, épaisseur du trait et police grasse
    $this->SetFillColor(0,0,0); // rouge 255.0.0
    $this->SetTextColor(255);
...
// Tableau coloré
function FancyTable_perte($header, $data)
{
    // Couleurs, épaisseur du trait et police grasse
    $this->SetFillColor(0,0,0); // rouge 255.0.0
    $this->SetTextColor(255);
